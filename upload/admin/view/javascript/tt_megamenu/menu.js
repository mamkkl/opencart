...
var tt_megamenu = {
    ...
    'submitTopItemForm' : function (menu_id) {
        ...
        $.ajax({
            ...
            success : function (json) {
                if(json['submit'] === true) {
                    tt_megamenu.getItemsToContainer(menu_id);
                } else {
                    flag = false;
                    $('#form-container-bg').show();
                    $('#form-ajax-loader').show();
            ...
            success     : function (json) {
                ...
                if(top_items) {
                    ...
                    for(i in top_items) {
                        ...
                        html += '           <a href="javascript:void(0)" onclick="tt_megamenu.appendTopItemNewForm(\'' + top_items[i]['url'] + '\')" class="a-config"><i class="fa fa-cogs"></i></a>';
                        html += '           <a href="javascript:void(0)" onclick="tt_megamenu.deleteMenuItem(\'' + top_items[i]['del_url'] + '\')" class="a-delete"><i class="fa fa-trash"></i></a>';
                        html += '           <a href="javascript:void(0)" onclick="tt_megamenu.appendAddSubItemForm(\'' + top_items[i]['menu_item_id'] + '\', \'2\')" class="a-config"><i class="fa fa-plus"></i> ' + text_add_sub_item + '</a>';
                        html += '       </div>';
                        html += '   </div>';
                        ...
                        if(top_items[i]['sub_items']) {
                            ...
                            for(j in sub_items) {
                                ...
                                html += '       </div>';
                                html += '       <div class="action col-sm-6">';
                                html += '           <a href="javascript:void(0)" onclick="tt_megamenu.appendEditSubItemForm(\'' + sub_items[j]['item_id'] + '\')" class="a-config"><i class="fa fa-cogs"></i></a>';
                                html += '           <a href="javascript:void(0)" onclick="tt_megamenu.deleteMenuItem(\'' + sub_items[j]['del_url'] + '\')" class="a-delete"><i class="fa fa-trash"></i></a>';
                                html += '           <a href="javascript:void(0)" onclick="tt_megamenu.appendAddSubItemForm(\'' + sub_items[j]['item_id'] + '\', \'3\')" class="a-config"><i class="fa fa-plus"></i> ' + text_add_sub_item + '</a>';
                                html += '       </div>';
                                html += '   </div>';
                                ...
                                if(sub_items[j]['sub_items']) {
                                    ...
                                    for(k in s_items) {
                                        ...
                                        html += '       </div>';
                                        html += '       <div class="action col-sm-6">';
                                        html += '           <a href="javascript:void(0)" onclick="tt_megamenu.appendEditSubItemForm(\'' + s_items[k]['item_id'] + '\')" class="a-config"><i class="fa fa-cogs"></i></a>';
                                        html += '           <a href="javascript:void(0)" onclick="tt_megamenu.deleteMenuItem(\'' + s_items[k]['del_url'] + '\')" class="a-delete"><i class="fa fa-trash"></i></a>';
                                        html += '       </div>';
                                        html += '   </div>';
            ...
            success : function (json) {
                ...
                if(child_categories) {
                    for(i in child_categories) {
                        ...
                        html += '           <a class="a-resize-minus a-left" onclick="tt_megamenu.decreaseColumn($(this))"><i class="fa fa-chevron-left" aria-hidden="true"></i></a>';
                        html += '           <a class="a-resize-plus a-left" onclick="tt_megamenu.increaseColumn($(this))"><i class="fa fa-chevron-right" aria-hidden="true"></i></a>';
                        html += '           <a class="a-delete a-right" onclick="tt_megamenu.deleteWidget($(this))"><i class="fa fa-times-circle" aria-hidden="true"></i></a>';
                        html += '           <a class="a-config a-right" onclick="tt_megamenu.showWidgetConfiguration($(this))"><i class="fa fa-cogs" aria-hidden="true"></i></a>';
                        html += '       </div>';
    ...
    'addWidget': function() {
        ...
        if(widget_type == 'html') {
            ...
            $.ajax({
                url : 'index.php?route=extension/module/tt_megamenu/getLanguageData&user_token=' + user_token,
                type: 'get',
                success : function (json) {
        ...
        tt_megamenu.initSortMenuItem();
        tt_megamenu.chooseCategory();
        tt_megamenu.chooseProduct();
        tt_megamenu.initSummerNote();
        tt_megamenu.checkBoxClick();
    },
    ...
    'chooseCategory' : function () {
        ...
        $('.w-category').customAutoComplete({
            'source': function(request, response) {
                $.ajax({
                    url: 'index.php?route=extension/module/tt_megamenu/autoCompleteCategory&user_token=' + user_token + '&filter_name=' +  encodeURIComponent(request),
                    dataType: 'json',
                    success: function(json) {
